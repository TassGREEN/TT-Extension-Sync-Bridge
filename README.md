...
设置页的“生成并复制诊断日志”会显示并尝试复制一份实时脱敏 JSON。它包含 Bridge/目标插件版本、快照元数据、最近状态，以及酒馆助手脚本树的结构计数和三个固定目标 ID 的 found/missing 结果；不含 snapshot payload、脚本正文、Provider、口令、聊天或会话数据。手机剪贴板不可用时可长按日志文本框全选复制。
