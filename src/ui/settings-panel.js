...
export function mountBridgeSettingsPanel(runtime) {
  ...
  root.innerHTML = `
    <div class="inline-drawer">
      ...
      <div class="inline-drawer-content">
        ...
        <label class="checkbox_label ttsb-disabled"><input data-setting="sensitiveDataSync" type="checkbox" disabled> 敏感数据同步（需要加密，当前版本强制关闭）</label>
  ...
  function syncControls() {
    ...
    root.querySelector('[data-setting="sensitiveDataSync"]').checked = false;
  ...
  root.querySelector('[data-action="capture"]').addEventListener('click', async () => {
    ...
    const results = await busy('正在采集并写入 Extension Store…', () => (
      runtime.controller.captureAll(enabledAdapterIds(), { includeSensitive: false })
    ));
  ...
  root.querySelector('[data-action="preview"]').addEventListener('click', async () => {
    ...
    previews = await busy('正在验证快照并生成恢复预览…', async () => {
      ...
      for (const adapterId of enabledAdapterIds()) {
        try {
          results.push(await runtime.controller.previewRestore(adapterId));
        } catch (error) {
    ...
    summary.textContent = `预览完成：${changes} 项待恢复，${conflicts} 项冲突，${failures} 项失败。`;
  ...
  restoreButton.addEventListener('click', async () => {
    ...
    if (!globalThis.confirm(`将恢复 ${candidates.length} 个 adapter。设备本地敏感字段会保留，确定继续吗？`)) return;
